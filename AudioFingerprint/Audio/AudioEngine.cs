...
namespace AudioFingerprint.Audio
{
    public class AudioEngine : IDisposable
    {
        ...
        {
            var absEnergy = new double[(Frequencies.Length - 1) * 4]; // number of frequencies bands (33)
            var lowpassFilters = new LowpassFilters(Frequencies.Length - 1); // 33 bands
            try
            {
                ...
                for (int i = 0; i < width; i++)
                {
                    ...
                    byte[] reliability;
                    uint subFingerprint = CalculateSubFingerprint(complexSignal, absEnergy, lowpassFilters, frequenciesRange, 5512, out reliability);
        ...
        private uint CalculateSubFingerprint(float[] spectrum, double[] absEnergy, LowpassFilters lowpassFilters, int[] frequenciesRange, int sampleRate, out byte[] reliability)
        {
