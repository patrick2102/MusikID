...
namespace AudioFingerprint.Audio
{
    public class FingerprintSignature : IDisposable
    {
        ...
        private long ID = 0; // Finger ID
        private object reference = null; // identifier
        private object tag = null;

        private int diskotekNr;
        private int sideNr;
        ...
        private long duration = -1; // this is the "Real" durarion as calculate from orginal audio file
        private string audioFormat = string.Empty;
            ...
            : this(null, 0, signature, 0)
        {
            ...
            duration = (int)(subFingerprintCount * 11.6);
        }

        public FingerprintSignature(byte[] signature, long duration)
            : this(null, 0, signature, duration)
        {
        }

        public FingerprintSignature(object reference, int diskotekNr, int sideNr, int sequenceNr, long ID, byte[] signature, long duration, bool createLookupIndex = false)
        {
        ...
        public FingerprintSignature(object reference, long ID, byte[] signature, long duration, bool createLookupIndex = false)
        {
            AudioFingerprint.Math.SimilarityUtility.InitSimilarityUtility();
        ...
        public FingerprintSignature(object reference, long ID, byte[] signature, byte[] mmHashtableData, long duration)
        {
            this.ID = ID;
